  ...
  wassDivEnt <- R6::R6Class("wassDivEnt", 
                           inherit = cgOptimizer,
                             public = list(
                               ...
                               #   aXb <-  matrix(private$a, private$n1, private$n2) * matrix(private$b, private$n1, private$n2, byrow = TRUE)
                               #   
                               #   return(
                               #     c(sum(private$f_pot * private$a) + sum(private$g_pot * private$b) -
                               #       private$lambda * sum(aXb * E))
                               #   )
                               # },
                               # df = function() {
                               #   fmat <- matrix(private$f_pot, private$n1, private$n2)
                               ...
                               #   
                               #   E    <- exp((fmat + gmat - private$cost)/private$lambda) - 1
                               #   noaXb <-  matrix(private$b, private$n1, private$n2, byrow = TRUE)
                               #   
                               #   return(
                               #     c(private$f_pot - private$lambda * rowSums(noaXb * E) )
                               #   )
                               # },
                               return_cw = function() {
                            ...
                            public = list(
                              ...
                              step = function() {
                                ...
                                if (-derphi0 >  private$tol) {
                                  ...
                                  if ( !is.null(search_res$alpha)) {
                                    search_res$alpha = min(1, max(search_res$alpha,0))
                                    private$a <- simplex_proj(old_a + search_res$alpha * deltaG)
                                  }
                                  
                                } else {
                                  private$a <- old_a
                                }
                                private$optimizer_xy$update_a(private$a)
                                private$optimizer_xx$update_a(private$a)
                              },
                              initialize = function(X1, X2, 
                                                    cost = NULL,
                                                    qp_solver = c("mosek", "gurobi", 
