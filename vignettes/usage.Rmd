...
#> Error in if (opt_args$line_search_fn != "strong_wolfe") warning("line_search_fn = 'strong_wolfe' recommended for the torch LBFGS optimizer. You can pass the `line_search_fn` arg as an additional argument to this function."): argument is of length zero
print(tau_hat_bp@estimate)
#> Error in print(tau_hat_bp@estimate): object 'tau_hat_bp' not found
```
...
```r
vcov(tau_hat_bp)
#> Error in vcov(tau_hat_bp): object 'tau_hat_bp' not found
vcov(tau_hat_wols)
#>            estimate
#> estimate 0.01194476
vcov(tau_hat)
...
```
This can be passed to the `calc_weight` function directly to the argument `x`.
