...
```
Note that the calc_weight function will not use the outcome data `y` when calculating the weights but will pass it to the internal data constructor.
...
weights <- calc_weight(x = x, z = z, method = "COT",
                       estimand = "ATE", options = list(lambda.bootstrap = Inf))
...
tau_hat <- estimate_effect(causalWeights = weights,
                           y = y)
```
The estimator generated here is a simple weighted difference in observed outcomes between treatment groups. Moreover, note we must supply the outcome information in argumnet `y` since we didn't provide it in the `calc_weight` function.
...
preds   <- predict(object = bp, 
                   ...
                   source.sample = df$z)
head(preds)
#> [1] -2.8074944  0.5236986  5.8073131 -1.5503667  1.4414216 -2.4308861
```
...
#>          X1          X2          X3          X4          X5          X6 
#> 0.024558231 0.028086483 0.032456111 0.011734582 0.006399137 0.012215878
```
Pretty good!
...
#> Control group
#>                                pre        post
#> OT distance              0.5311378  0.00187791
#> Pareto k                        NA  0.75983757
#> N eff                  247.0000000 27.49105164
#> Avg. std. mean balance   0.3067516  0.01676450
#> 
#> Treated group
#>                                pre         post
...
#> Error in `ggplot2::geom_line()` at ]8;line = 170:col = 4;file:///Users/eifer/GoogleDrive/Research/Jose Zubizarreta Research/causalOT/R/summary.RcausalOT/R/summary.R:170:4]8;;:
